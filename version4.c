...
bool habitat_valide(const char *habitat) {
    ...
    for (int i = 0; i < n; i++) {
        if (stricmp(habitats[i], habitat) == 0) {
            return true;
        }
    }
    return false;
...
void ajouter(Animal tab_zoo[], int *taille) {
    system("cls");
    int n;
    ...
    for (int i = *taille; i < *taille + n; i++) {
        ...
        printf("Le nom : ");
        scanf("%s", tab_zoo[i].nom);
        
        printf("L'espece : ");
        scanf("%s", tab_zoo[i].espece);
...
void afficher(Animal tab_zoo[], int *taille) {
    ...
    int choix;
    printf("\n____________AFFICHAGE_____________\n");
    printf("1- Par ID\n");
    printf("2- Par Nom\n");
    printf("3- Par age \n");
    ...
    if (choix == 1) {
        ...
        for (int i = 0; i < *taille - 1; i++) {
            for (int j = i + 1; j < *taille; j++) {
                if (stricmp(temp[i].nom, temp[j].nom) > 0) {
                    Animal t = temp[i];
                    temp[i] = temp[j];
                    temp[j] = t;
                }
        ...
        for (int i = 0; i < *taille - 1; i++) {
            for (int j = i + 1; j < *taille; j++) {
                if (stricmp(temp[i].habitat, temp[j].habitat) > 0) {
                    Animal t = temp[i];
                    temp[i] = temp[j];
                    temp[j] = t;
                }
...
void chercher_nom(Animal tab_zoo[], int *taille) {
    ...
    for (int i = 0; i < *taille; i++) {
        if (stricmp(nom_recherche, tab_zoo[i].nom) == 0) {
            trouve = 1;
            printf("\n======== Animal trouve =========\n");
            printf("ID: %d\n", tab_zoo[i].id);
            printf("Nom: %s\n", tab_zoo[i].nom);
...
void chercher_espece(Animal tab_zoo[], int *taille) {
    ...
    for (int i = 0; i < *taille; i++) {
        if (stricmp(espece_recherche, tab_zoo[i].espece) == 0) {
            trouve = 1;
            printf("\n======== Animal trouve =========\n");
            printf("ID: %d\n", tab_zoo[i].id);
            printf("Nom: %s\n", tab_zoo[i].nom);
...
void rech(Animal tab_zoo[], int *taille) {
    ...
    printf("\n____________________RECHERCHE_________________\n");
...
void espece_majoritaire(Animal tab_zoo[], int taille) {
    ...
    for (int i = 0; i < taille; i++) {
        ...
        for (int j = 0; j < taille; j++) {
            if (stricmp(tab_zoo[i].espece, tab_zoo[j].espece) == 0) {
                count++;
            }
        }
        if (count > max_count) {
...
int main() {
    ...
    do {
        ...
        switch (choix) {
            case 1: ajouter(tab_zoo, &taille); break;
